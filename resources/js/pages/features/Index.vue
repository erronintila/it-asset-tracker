<template>
    <div>
        <page-header backButton title="Review Category Features">
            <template slot="leftSideNavigation">
                <v-btn
                    text
                    title="Add new feature"
                    @click="dialogFeatureCreate = true"
                    icon
                >
        ...
        <!-- Dialogs -->
        <FeatureCreate
            :dialogCreate="dialogFeatureCreate"
            :review_category_id="$route.params.id"
            @close-dialog="dialogFeatureCreate = false"
            @save-dialog="
                () => {
                    getData();
...
import FeatureCreate from "./Create.vue";
...
export default {
    ...
    methods: {
            ...
            return new Promise((resolve, reject) => {
                ...
                let data = {
                    params: {
                        ...
                        search: search,
                        status: status,
                        review_category_id: this.$route.params.id
                    }
                };

                FeatureDataService.getAll(data)
                    .then(response => {
        ...
        onDelete: function() {
            ...
            FeatureDataService.deleteMany(data)
                .then(response => {
                    this.getData();
                    this.selectedItems = [];
                })
        ...
        onRestore: function() {
            ...
            FeatureDataService.restore(data)
                .then(response => {
                    this.getData();
                    this.selectedItems = [];
                })
